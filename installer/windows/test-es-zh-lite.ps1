...
$FixtureRoot = Join-Path $Root '.build\es-zh-lite-smoke'
...
try {
    ...
    & $Python $EngineMain models `
        ...
        install lite-es-zh --download-only | Out-Host
    if ($LASTEXITCODE -ne 0) { throw 'No se pudo preparar lite-es-zh.' }
...
print('ES_ZH_LITE_PACK', f'{pack.id}@{pack.version}')
print('ES_ZH_ROUTE', report['route'])
print('ES_ZH_ASR_P95_MS', report['asrP95Ms'])
print('ES_ZH_E2E_P95_MS', report['endToEndP95Ms'])
print('ES_ZH_RTF_P95', report['combinedRtfP95'])
print('ES_ZH_ENGINE_PEAK_MB', report['enginePeakWorkingSetMb'])
print('ES_ZH_TOTAL_PRODUCT_MB', report['totalProductWorkingSetMb'])
if report['route'] != 'es-zh':
    raise SystemExit('ES_ZH_ROUTE_MISMATCH')
if report['totalProductWorkingSetMb'] > 1536.0:
    raise SystemExit('ES_ZH_LITE_PRODUCT_MEMORY_LIMIT')
if report['combinedRtfP95'] >= 0.80:
    raise SystemExit('ES_ZH_LITE_RTF_LIMIT')
if report['endToEndP95Ms'] > 1500.0:
    raise SystemExit('ES_ZH_LITE_LATENCY_LIMIT')
if not report['passed']:
    raise SystemExit('ES_ZH_LITE_GATE_FAILED: ' + ','.join(report['failures']))
print('ES_ZH_LITE_GATE_OK')
'@ | Set-Content -Path $Probe -Encoding UTF8

    & $Python $Probe $EngineApp $ModelsRoot $ReportPath
    if ($LASTEXITCODE -ne 0) { throw 'ES→ZH Lite no pasó benchmark realtime/memoria total.' }
    if (-not (Test-Path $ReportPath -PathType Leaf)) { throw 'ES→ZH Lite no produjo reporte JSON.' }

    Write-Host "ES→ZH LITE OK: $ReportPath" -ForegroundColor Green
}
finally {
    Remove-Item $FixtureRoot -Recurse -Force -ErrorAction SilentlyContinue
}
