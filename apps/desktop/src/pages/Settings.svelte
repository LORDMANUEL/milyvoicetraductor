...
<section class="page-stack">
  ...
  <div class="two-columns">
    <article class="panel-card form-card">
      ...
      <label>Puerto local<input type="number" min="1024" max="65535" bind:value={draft.enginePort}></label>
    ...
    <article class="panel-card"><span class="card-title">Caché regenerable</span><strong class="big-value small">{cache.entries} entradas</strong><p>Los modelos y transcripciones no se borran con esta acción.</p><button class="secondary" onclick={clearCache}>Limpiar caché</button><hr><h3>Privacidad</h3><p>El audio no se guarda. La persistencia de texto está desactivada hasta que la habilites.</p></article>
  </div>
</section>
