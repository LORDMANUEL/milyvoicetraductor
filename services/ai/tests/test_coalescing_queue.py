...
class CoalescingTranslationQueueTests(unittest.IsolatedAsyncioTestCase):
    async def test_new_partial_replaces_old_partial_for_same_utterance(self):
        ...
        self.assertTrue(await queue.put(request("u1", "hello", created_at=0.1)))
        self.assertTrue(await queue.put(request("u1", "hello world", created_at=0.2)))
    ...
    async def test_final_removes_partial_and_has_priority(self):
        ...
        await queue.put(request("u2", "partial two", created_at=0.2))
        await queue.put(request("u1", "final one", final=True, created_at=0.3))
    ...
    async def test_stale_partial_is_evicted_before_enqueue(self):
        ...
        self.assertEqual(queue.qsize(), 1)
        self.assertEqual((await queue.get()).original, "fresh")
    ...
    async def test_full_queue_never_drops_final(self):
        ...
        self.assertTrue(
            await queue.put(request("u2", "important", final=True, created_at=1.0))
        )
