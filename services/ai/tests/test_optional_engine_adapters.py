        ...
        return types.SimpleNamespace(
            results=[
                types.SimpleNamespace(
                    alternatives=[
                        types.SimpleNamespace(transcript="hello from cloud")
                    ],
                    language_code="en-US",
                )
            ]
...
class OptionalEngineAdapterTests(unittest.TestCase):
    def test_vosk_keeps_model_loaded_and_returns_words(self):
        ...
        with tempfile.TemporaryDirectory() as tmp, patch.dict(
            sys.modules, {"vosk": FakeVoskModule}
        ):
            ...
            self.assertEqual(
                [word.text for word in first[0].words], ["hello", "world"]
            )
    ...
    def test_moonshine_passes_model_arch_and_disables_audio_return(self):
        ...
        with tempfile.TemporaryDirectory() as tmp, patch.dict(
            sys.modules, {"moonshine_voice": fake_module}
        ):
            root = Path(tmp)
            (root / "moonshine-config.json").write_text(
                json.dumps({"modelArch": 2, "updateInterval": 0.25}),
                encoding="utf-8",
            )
        ...
        self.assertEqual(kwargs["options"]["return_audio_data"], "false")
    ...
    def test_google_chirp_builds_chirp3_v2_request(self):
        ...
        with patch.dict(
            os.environ,
            {
                ...
                "MILY_GOOGLE_CLOUD_REGION": "us",
                "MILY_GOOGLE_CHIRP_CONSENT": "1",
            },
            clear=True,
        ):
            provider = GoogleChirpV2Asr(
                Path("."), client=client, speech_types=FakeCloudSpeech
            )
        ...
        self.assertEqual(
            request.kwargs["recognizer"],
            "projects/demo/locations/us/recognizers/_",
        )
