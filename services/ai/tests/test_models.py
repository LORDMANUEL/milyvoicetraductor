...
class ModelManagerTests(unittest.TestCase):
    def fake_snapshot(self, repo_id, revision, local_dir):
        ...
        (target / "config.json").write_text(f'{{"repo":"{repo_id}","revision":"{revision}"}}', encoding="utf-8")
        return str(target)
    ...
    def test_install_is_atomic_and_marks_pack_active(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse((catalog.models_dir / ".staging" / "business-qwen-1.0.0").exists())
            self.assertTrue(installer.verify(pack.id, pack.version))
            (pack.path / "components" / "asr" / "config.json").write_text("corrupt", encoding="utf-8")
