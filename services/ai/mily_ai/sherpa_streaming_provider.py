"""Sherpa-ONNX streaming con audio incremental y presupuesto CPU mínimo.

El pipeline de MilyVoice entrega ventanas acumulativas durante una utterance. Este
adaptador conserva recognizer/stream nativos y envía únicamente muestras nuevas.
Para packs sherpa antiguos sin ``sherpaMode`` mantiene el adapter offline legado.
"""
...
class SherpaStreamingAsr(LegacySherpaOnnxAsr):
    """Online Zipformer/Paraformer residente y limitado por CpuBudget."""
    ...
    def __init__(
        ...
        *,
        ...
        self.fallback_reason = (
            "Sherpa Lite está certificado en CPU."
    ...
    def _component_path(self, key: str) -> str:
        ...
        if not value:
            raise OptionalProviderRuntimeError(
                "SHERPA_CONFIG_INVALID", f"El pack sherpa no declara {key}."
            )
    ...
    def _load_online(self):
        ...
        try:
            ...
            raise OptionalProviderRuntimeError(
                ...
                "sherpa-onnx no está instalado en el runtime privado.",
            ) from exc
        ...
        try:
            if self._mode == "online-transducer":
                ...
                raise OptionalProviderRuntimeError(
                    "SHERPA_CONFIG_INVALID", "El modo sherpa streaming no es válido."
                )
    ...
    def _starts_new_utterance(self, samples: Sequence[float]) -> bool:
        ...
        if prefix_len <= 0:
            return False
        return tuple(float(v) for v in samples[:prefix_len]) != self._utterance_prefix[:prefix_len]
        ...
        if self._online_stream is None:
            ...
            try:
                ...
                self._utterance_prefix = tuple(
                    float(v) for v in samples[: self._PREFIX_SAMPLES]
                )
            except Exception as exc:
                self._close_online_stream()
                raise OptionalProviderRuntimeError(
    ...
    def _segments(self, text: str, source_language: str, audio_samples: int) -> list[AsrSegment]:
    ...
    def _accept_delta(self, stream: Any, normalized: list[float]) -> None:
        ...
        try:
            ...
            self._close_online_stream()
            raise OptionalProviderRuntimeError(
                "SHERPA_STREAM_AUDIO", "sherpa-onnx no pudo recibir audio realtime."
            ) from exc
    ...
    def _online_transcribe(self, samples: Sequence[float], source_language: str) -> list[AsrSegment]:
        normalized = [float(v) for v in samples]
        if not normalized:
            return []
    ...
    def transcribe_final(self, samples: Sequence[float], source_language: str) -> list[AsrSegment]:
        if not self._is_online:
            return super().transcribe(samples, source_language)
        normalized = [float(v) for v in samples]
        if not normalized:
