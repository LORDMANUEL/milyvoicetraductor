"""Deterministic translation quality/fidelity guards extracted from MilyVoice 2.x.

This module intentionally preserves the established behavior while making it
available behind the independent Linguistic component boundary.
"""
...
def analyze_source_target_fidelity(
    ...
    source_has_negation = (
        bool(_EN_NEGATION_RE.search(source_normalized))
        if source_language == "en"
        else False
    )
    target_has_negation = (
        bool(target_tokens & _ES_NEGATIONS) if target_language == "es" else False
    )
    if (
        source_language == "en"
        and target_language == "es"
        and source_has_negation
        and not target_has_negation
    ):
        return SourceTargetFidelity(
            missing_numbers=(),
...
def non_repetitive_sentence_prefix(text: str) -> str:
    ...
    parts = [
        part.strip()
        for part in _SENTENCE_RE.findall(str(text or ""))
        if part.strip()
    ]
    accepted: list[str] = []
    for part in parts:
