...
class _PyAudioPatchBackend:
    """Adapt PyAudioWPatch module constants to a PyAudio instance."""
...
class WasapiLoopbackSource:
    """Normalize WASAPI loopback to mono float PCM at the target sample rate.

    If the current Windows loopback remains silent, alternative loopback devices
    are probed and the source changes only when another device produces real
    activity above the configured RMS threshold.
    """
    ...
    def _decode_chunk(self, raw: bytes, info: LoopbackDeviceInfo) -> np.ndarray:
        ...
        return self._resample_mono(
            samples, info.sample_rate, self.target_sample_rate
        )
    ...
    def _probe_active_alternative(self) -> np.ndarray | None:
        ...
        for raw in self._loopback_devices:
            try:
                ...
                # A broken alternative must never take down the valid source.
                continue
    ...
    def read_chunk(self) -> list[float]:
        if self._stream is None or self._device is None:
            raise LoopbackError(
                "LOOPBACK_DEVICE", "El audio del sistema no está iniciado."
            )
        try:
            raw = self._stream.read(
                self._input_frames, exception_on_overflow=False
            )
