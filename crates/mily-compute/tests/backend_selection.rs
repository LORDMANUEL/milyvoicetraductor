use mily_compute::{BackendObservation, ComputeBackend, select_best_backend};
...
fn fastest_successful_measured_backend_wins_even_if_it_is_not_cuda() {
    ...
    assert_eq!(select_best_backend(&observations), ComputeBackend::DirectMl);
}
...
fn rtf_is_primary_and_latency_breaks_close_ties() {
    ...
    assert_eq!(select_best_backend(&observations), ComputeBackend::OpenVino);
}
